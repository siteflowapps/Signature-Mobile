package com.siteflow.cdo.ase.dashboard.data
...
import com.siteflow.cdo.core.util.toTitleCase

/**
 * The 5 steps of CDO (Complete Definition of an Outlet).
 *
 ...
 * 5. CDO Verify      — Branding + Cooler installed & verified with photos (ASE)
 */
enum class CdoStep(val label: String) {
    ENROLLMENT("Enrollment"),
    ASM_APPROVAL("ASM Approval"),
    ASSET_REQUEST("Asset Request"),
    CDO_VERIFICATION("CDO Verification")
}
...
data class OutletItem(
    ...
    val completedSteps: Set<CdoStep> = emptySet(),
    ...
    val cdoVerificationPhotos: List<PhotoSlot> = emptyList(),
    ...
    val completionPercent: Int
        get() = ((completedSteps.size.toFloat() / CdoStep.entries.size) * 100).toInt()

    val nextPendingStep: CdoStep?
        get() = CdoStep.entries.firstOrNull { it !in completedSteps }
    ...
    companion object {
         ...
         */
        fun fromDto(dto: com.siteflow.cdo.ase.onboarding.data.dto.OutletResponseData): OutletItem {
        ...
        /**
         * Derives completed CDO steps from the outlet's status and asset status.
        ...
        private fun completedStepsFromStatus(
            ...
            val steps = mutableSetOf<CdoStep>()
            when (status) {
                OutletStatus.DRAFT_BASIC,
                OutletStatus.DRAFT_BUSINESS_DETAILS,
                OutletStatus.DRAFT_KYC,
                ...
                OutletStatus.ASM_PENDING -> steps.add(CdoStep.ENROLLMENT)
                OutletStatus.ASM_REJECTED -> steps.add(CdoStep.ENROLLMENT)
                OutletStatus.ASM_APPROVED -> {
                    steps.add(CdoStep.ENROLLMENT)
                    steps.add(CdoStep.ASM_APPROVAL)
                }
                OutletStatus.ONBOARDED -> return CdoStep.entries.toSet()
            }
            // Asset request step
            if (assetStatus != AssetStatus.NOT_REQUESTED) {
                steps.add(CdoStep.ASSET_REQUEST)
            }
            // CDO Verification step — complete only when ASM has verified (VERIFIED status)
            if (assetStatus == AssetStatus.VERIFIED) {
                steps.add(CdoStep.CDO_VERIFICATION)
            }
            return steps
        }
...
val mockOutlets = listOf(
    OutletItem(
        ...
        completedSteps = setOf(CdoStep.ENROLLMENT),
    ...
    OutletItem(
        ...
        completedSteps = setOf(CdoStep.ENROLLMENT, CdoStep.ASM_APPROVAL, CdoStep.ASSET_REQUEST),
    ...
    OutletItem(
        ...
        completedSteps = CdoStep.entries.toSet(),
        ...
        cdoVerificationPhotos = listOf(
            PhotoSlot("cdo_front", "CDO Front", true, "/mock/cdo_front.jpg"),
            PhotoSlot("cdo_inside1", "Inside View 1", false, "/mock/cdo_in1.jpg"),
            PhotoSlot("cdo_inside2", "Inside View 2", false, "/mock/cdo_in2.jpg"),
            PhotoSlot("cdo_cooler", "Cooler Installed", false, "/mock/cdo_cool.jpg"),
            PhotoSlot("cdo_branding", "Branding Completed", false, "/mock/cdo_brand.jpg")
        ),
        ...
        agreementAccepted = true,
        timeline = listOf(
            TimelineEntry("CDO Verified", "Feb 22, 10:00 AM"),
    ...
    OutletItem(
        ...
        completedSteps = setOf(CdoStep.ENROLLMENT, CdoStep.ASM_APPROVAL),
    ...
    OutletItem(
        ...
        dmsId = "PCA-2024-998",
        cdoVerificationPhotos = emptyList(), // Intentionally empty to show "Upload CDO photos"
        timeline = listOf(
